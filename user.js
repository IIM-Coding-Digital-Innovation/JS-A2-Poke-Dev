...
if(document.URL.includes("register.html")){
    ...
    thisForm.addEventListener("submit", function(event) {
        ...
        if (!submitBtn.disabled) {
            ...
            .then((resp) => {
            console.log("success!")
            Cookies.set('username', document.querySelector("#Name").value, { expires: 7 });
...
if(document.URL.includes("login.html")){
 ...
 loginForm.addEventListener("submit", function(event) {
     ...
     axios.get("https://api.airtable.com/v0/appDaIOPzRpXiYeF2/Table%201", { headers: { 'Authorization': 'Bearer YOUR_API_KEY' } })
         .then((response) => {
             // Check if the username and password match the data in the Airtable
             const records = response.data.records;
             let match = false;
             records.forEach(record => {
                 if (record.fields.username === username && record.fields.password === password) {
                     match = true;
                 }
             });
